...
sem_t *MakeSemaphore(const char *path) {
  ...
  if (sema == SEM_FAILED) {
    throw std::runtime_error{
        std::format("Opening semaphore failed, error: {}", strerror(errno))};
  }
...
int main() {
  static constexpr auto ShmFile = "/kafka";
  ...
  CHECK_ERROR(ftruncate(shmFd, SharedData::DataSize));

  void *sharedMemoryPtr = mmap(nullptr, SharedData::DataSize,
  ...
  if (pid == 0) {
    while (true) {
      ...
      std::cout << std::format("[CHILD]: read message of size: {}",
                               sharedData->m_messageSize)
                << std::endl;
      std::cout << std::format("[CHILD]: content: {}", sharedData->buffer)
                << std::endl;
    ...
    while (std::getline(std::cin, input)) {
      ...
      std::memcpy(sharedData->buffer, input.data(),
                  std::min(input.size(), SharedData::BufferSize));

      std::cout << std::format("[PARENT]: wrote {} bytes", input.size())
                << std::endl;
    ...
    CHECK_ERROR(shm_unlink(ShmFile));
  }
